...
This event is sent from other NApps with cookie information.
...
Im simple terms, enabling a pipeline will mean to modify every flow so it complies with the pipeline configuration. The process to successfully enable a pipeline is as follows:
  ...
  5. **of_multi_table** duplicates every flow that needs a different table with NApps cookie information from **kytos/<napp_name>.cookie**.
