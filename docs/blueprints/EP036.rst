...
Also, a new NApp **of_multi_table** that will initially have the following responsibilities:
  ...
  - Store cookie prefixes from NApp that publish flows.
...
-----------
  ...
  - **kytos/[mef_eline|telemetry_int|coloring|of_lldp].enable_table** - Every NApp sends an event with their table group settings and cookie prefix.

kytos/<napp_name>.enable_table
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

This event is sent from other NApps with information about their table settings and their cookie prefix.

.. code:: javascript

  "content": {
    "group_table": {"epl": 3, "evpl": 2},
    "cookie": "0xAA"
  }
...
Im simple terms, enabling a pipeline will mean to modify every flow so it complies with the pipeline configuration. The process to successfully enable a pipeline is as follows:
  ...
  5. **of_multi_table** duplicates every flow that needs a different table with NApps cookie prefix from **kytos/<napp_name>.enable_table**.
