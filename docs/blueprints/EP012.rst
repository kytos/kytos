...
.. code:: bash
  ...
  {str bandwidth, datetime start_date, datetime end_date, 
...
For example, suppose you have an EVC with ``start_date`` at 4pm and 
``end_date`` at 6pm. At 3pm it is edited and ``start_date`` is changed
to 2pm. In this case, the EVC should be imediately deployed.
If ``primary_links`` or ``backup_links`` are modified then new paths
must be found and redeployed if necessary.
